...
import Pop from '@c/icons/iconPor';
  ...
  render() {
    return (
      <div>
        ...
        <CSSTransition
          in={this.state.showPermissionModal}
          key='test'
          timeout={200}
          unmountOnExit
          classNames="figure">
          ...
          }} type={'permission'} title="用户授权" >
            <div className={Css.permissionModalContent}>
              <button
                className={Css.permissionModalContentBtn}
                onClick={() => this.getDevicePermission().finally(() => {
                this.setState({
                  showPermissionModal: false,
                })
              })}>点击授权重力感应</button>
